...
// Incident No.        : Orgcharts: hist. data (older 12 months)(3100077738)                     //
// CR No               : 1100060563                                                              //
// CD No               : 1400024591                                                              //
// Date                : 30-Jan-2017                                                             // 
// Description         : Creating PDF for Historic Data                                          //
//***********************************************************************************************//
...
namespace REORGCHART
{
    ...
    { 
        ...
        public ObjectInfPDF(string pId, string pTitle, string pPId,
                         ...
                         string pColorFlag, string pBackColor, string pFlag)
        {
            Id = pId;
            Title = pTitle;
            PId = pPId;
    ...
    public class AllLevelPDF
    {
                           ...
                           string TemplateName, string DownloadType, string PositionTCF, string PositionCF, string CN)
        {
            ...
            dtHD_DIRECT_REPORT = CompanyName.ToUpper()+"_DIRECT_REPORT";
            dtHD_DIRECT_REPORT_DATA = CompanyName.ToUpper() + "_DIRECT_REPORT_DATA";
            dtHD_DIRECT_REPORT_LV = CompanyName.ToUpper() + "_DIRECT_REPORT_LV";
            dtHD_DIRECT_REPORT_DATA_LV = CompanyName.ToUpper() + "_DIRECT_REPORT_DATA_LV";
        ...
        {
            int intIdx = 0, intIdy = 0, intIdz = 0;
        ...
        public string CreateAllLevelPDFIntermediateResults(string ShowType, string VersionNo, string TemplateName, string DownloadType)
        {
            string sQuery = "", sID = "";
            ...
            string dtORG_TABLE = UserData.CompanyName + "_";
        ...
        private List<ObjectInf> GetLevelInfo()
        {
            ...
            string InfoPos = "", ShowLevel = Level;
            ...
            DataTable dtlbl = LI.GetOrgChartDataTable(myla.Role, myla.Country, ShowLevel, myla.Levels, myla.Oper, myla.Version);
        ...
        private void PutFieldInfoPDF(string Info, int CurrentCol, int CurrentRow, int Width, int Height, Page MyPage, string ConnectorLineType)
        {
            int Idx = 0, Idy=0;
            string[] LabelInfo = Info.Replace("&amp;", "&").Split(';');
            string FontName = "", FontSize = "", FontColor = "", FontStyle = "", FontFloat = "", FontWidth = "", Adjustment = "";
            ...
            if (LabelInfo.Length >= 1)
            {
                ...
                {
                    MyLine = new ceTe.DynamicPDF.PageElements.Line(CurrentCol+150, CurrentRow-30, CurrentCol+150, CurrentRow, GetPDFLineColor(LineColor));
                    MyLine.Width = 2;
                    MyPage.Elements.Add(MyLine);
                }
                else if (ConnectorLineType == "B")
                {
                    MyLine = new ceTe.DynamicPDF.PageElements.Line(CurrentCol + 150, CurrentRow+80, CurrentCol + 150, CurrentRow + 100, GetPDFLineColor(LineColor));
                    MyLine.Width = 2;
                    MyPage.Elements.Add(MyLine);
                }
                foreach (DataRow drFI in dtFieldInf.Rows)
                {
                    try
                    {
                        ...
                        if (drFI["ALL_LEVEL_FLAG"].ToString() == "Y")
                        {
                            ...
                            MyLabel = new ceTe.DynamicPDF.PageElements.Label(LabelText[0].ToString(), CurrentCol, CurrentRow+((Idy-1)*18), Width, Height, 
                                                                             ceTe.DynamicPDF.Font.HelveticaBold, 15, 
                                                                             ceTe.DynamicPDF.TextAlign.Center);
        ...
        private int[] PutAssistanceFieldInfoPDF(int LevelId, int CurrentCol, int CurrentRow, int Width, int Height, Page MyPage)
        {
            int Idx = 0, Idy=0, Index=0, AssistanceCol=CurrentCol+400, AssistanceRow=CurrentRow-40;
            int[] retValue = { 0, 0 };
            string FontName = "", FontSize = "", FontColor = "", FontStyle = "", FontFloat = "", FontWidth = "", Adjustment = "";
                    ...
                    if (OI.PId != "-1" && OI.Flag=="Y")
                    {
                        ...
                        if (LabelInfo != null)
                        {
                            if (Index == 0)
                            {
                                ...
                                MyPage.Elements.Add(MyRect);
                                MyLine = new ceTe.DynamicPDF.PageElements.Line(CurrentCol-30, CurrentRow + 10, CurrentCol + 400, CurrentRow + 10, GetPDFLineColor(LineColor));
                                MyLine.Width = 2;
                                MyPage.Elements.Add(MyLine);
                            }
                            Index++;
                            ...
                            foreach (DataRow drFI in dtFieldInf.Rows)
                            {
                                try
                                {
                                    ...
                                    if (drFI["ALL_LEVEL_FLAG"].ToString() == "Y")
                                    {
                                        ...
                                        MyLabel = new ceTe.DynamicPDF.PageElements.Label(LabelText[0].ToString(), AssistanceCol, AssistanceRow + ((Idy - 1) * 18), Width, Height,
                                                                                         ceTe.DynamicPDF.Font.HelveticaBold, 15,
                                                                                         ceTe.DynamicPDF.TextAlign.Center);
                                        MyPage.Elements.Add(MyLabel);
            ...
            if (Idy >= 1)
            {
                retValue[0] = AssistanceCol + 400;
                retValue[1] = AssistanceRow + 100;
            }
            ...
            foreach (DataRow drInf in drObj)
            {
                ...
                foreach (ObjectInfPDF OI in theObjectInf)
                {
                    if (OI.PId != "-1" && OI.Flag=="N")
                    {
                        ...
                        PutFieldInfoPDF(OI.Title, CurrentCol, CurrentRow, 300, 80, MyPage, "V");
                        ...
                        if (OI.NOR.ToString() != "0")
                        {
                            ...
                            AssistanceInf = PutAssistanceFieldInfoPDF(Convert.ToInt32(OI.Id), CurrentCol, CurrentRow, 300, 80, MyPage);
        ...
        public int SelectLevelHorizantal(string LevelId, int CurrentCol, int MaxCol, FormattedTextAreaStyle style, Page MyPage, string TopNodeFlag)
        {
            ...
            foreach (DataRow drInf in drObj)
            {
                ...
                foreach (ObjectInfPDF OI in theObjectInf)
                {
                    if (OI.PId != "-1" && OI.Flag == "N")
                    {
                        ...
                        // Puts the data in the required area.
                        CurrentRow = 200;
                        PutFieldInfoPDF(OI.Title, CurrentCol, CurrentRow, 300, 80, MyPage, "H");
                        ...
                        if (OI.NOR.ToString() != "0")
                        {
                            ...
                            MaxCol = SelectLevelVertical(OI.Id, CurrentCol, MaxCol, style, MyPage);
            ...
            MyLine = new ceTe.DynamicPDF.PageElements.Line(StartCol + 150, StartRow - 30, MaxCol - 200, StartRow - 30, GetPDFLineColor(LineColor));
            MyLine.Width = 2;
            MyPage.Elements.Add(MyLine);
            ...
            {
                // Puts the data in the required area.
                PutFieldInfoPDF(TopNode.Title, (MaxCol - 300)/2, StartRow - 130, 300, 80, MyPage, "B");
            }
        ...
        public void CreateAllLevelPDF(DataTable OrgDataTable, string DownloadType, string CompanyName, string View, string FP)
        {
            dtHD_DIRECT_REPORT = CompanyName.ToUpper() + "_DIRECT_REPORT";
            dtHD_DIRECT_REPORT_DATA = CompanyName.ToUpper() + "_DIRECT_REPORT_DATA";
            dtHD_DIRECT_REPORT_LV = CompanyName.ToUpper() + "_DIRECT_REPORT_LV";
            ...
            PageDimensions MyPageDimensions = new PageDimensions(11000F, 8000F);
            Page MyPage = new ceTe.DynamicPDF.Page(MyPageDimensions);
            FormattedTextAreaStyle style = new FormattedTextAreaStyle(ceTe.DynamicPDF.FontFamily.Helvetica, 14, true);
            MyDocument.Pages.Add(MyPage);

            string sSQL = "", strCVIEW= "VIEW_DEFAULT";
            int CurrentCol=50, MaxCol=0;
            ...
            lstObjectPDF.Clear();
            DataRow[] OrgDataRow = OrgDataTable.Select("LEVEL_NO='1'");
            foreach (DataRow drLevel2 in OrgDataRow)
            {
                CurrentRow = 200;
                CurrentRow = SelectLevelHorizantal(drLevel2["LEVEL_ID"].ToString(), CurrentCol, MaxCol, style, MyPage, "Y");
                CurrentCol = MaxCol;
            }
            ...
            foreach (ObjectInfPDF Obj in lstObjectPDF)
            {
                if (Convert.ToInt32(Obj.NOR) >= 1)
                {
                    ...
                    {
                        MyLine = new ceTe.DynamicPDF.PageElements.Line(Obj.Width, Obj.Height, Obj.Owidth, Obj.Oheight - 20, GetPDFLineColor(LineColor));
                        MyLine.Width = 2;
                        MyPage.Elements.Add(MyLine);
                    }
                }
            ...
            //Outputs the MyDocument to the current web MyPage
            MyDocument.InitialPageZoom = PageZoom.FitPage;
            MyDocument.Draw(FP);
